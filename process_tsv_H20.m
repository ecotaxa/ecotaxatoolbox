function [base_Zooscan,m,zoo_groups,Idlistshort]=process_tsv_H20

%return
A=dir('*.tsv');
filenames={A.name};
[n,m]=size(filenames);
 T=readtable(char(filenames(1)),'Filetype','text','ReadVariableNames',1);

 samples=unique(T.object_htm_sample_barcode);
 m=length(samples);
...
for i=1:m
    ...
    tf = strcmp(T.object_htm_sample_barcode,samples(i));
    S=T(tf,:);

    sample=unique(S.object_htm_sample_barcode);
...
%     %base_Zooscan(i).StationId=str2num(cell2mat(unique(S.sample_stationid)));
   ...
   % Vol=unique(S.acq_sample_volumenormalised);
...
% zvoxelnew=0.003;
 ...
 %       I=strcmp(S.acq_id,base_Zooscan(i).FracIds(fracnb));
  I=strcmp(S.object_htm_sample_barcode,samples(i));
...
%         %%
%         eval(['base_Zooscan(i).d' num2str(fracnb) '.major=S.object_feature_major_axis(I)/1000*newvoxel/oldvoxel;']) %allready in micron => in mm
%         eval(['base_Zooscan(i).d' num2str(fracnb) '.minor=S.object_feature_minor_axis(I)/1000*newvoxel/oldvoxel;'])
%         %eval(['base_Zooscan(i).d' num2str(fracnb) '.area_exc=S.object_area_exc(I)*(pixelsize^2);'])
%         eval(['base_Zooscan(i).d' num2str(fracnb) '.summedbiovolume=S.object_feature_size(I)/1000/1000/1000*(newvoxel^2)/(oldvoxel^2)*(zvoxelnew)/(zvoxelold);']) %allready in cubed micron => in mm3
%         eval(['base_Zooscan(i).d' num2str(fracnb) '.area=S.object_feature_size2d(I)/1000/1000*(newvoxel^2)/(oldvoxel^2);'])    %allready in µm2 =>mm2
%         eval(['base_Zooscan(i).d' num2str(fracnb) '.summedarea=S.object_feature_size2d(I)/1000/1000*(newvoxel^2)/(oldvoxel^2);'])    %object__area
%         eval(['base_Zooscan(i).d' num2str(fracnb) '.ESD=2*(((S.object_feature_size2d(I)/1000/1000)/pi).^0.5)*(newvoxel^2)/(oldvoxel^2);'])    %object__area
        %%
%                 eval(['base_Zooscan(i).d' num2str(fracnb) '.major=S.object_feature_major_axis(I)/1000*2;']) %allready in micron => in mm
%         eval(['base_Zooscan(i).d' num2str(fracnb) '.minor=S.object_feature_minor_axis(I)/1000*2;'])
%         %eval(['base_Zooscan(i).d' num2str(fracnb) '.area_exc=S.object_area_exc(I)*(pixelsize^2);'])
%         eval(['base_Zooscan(i).d' num2str(fracnb) '.summedbiovolume=S.object_feature_size(I)/1000/1000/1000*(2*2*2);']) %allready in cubed micron => in mm3
%         eval(['base_Zooscan(i).d' num2str(fracnb) '.area=S.object_feature_size2d(I)/1000/1000*(2*2);'])    %allready in µm2 =>mm2
%         eval(['base_Zooscan(i).d' num2str(fracnb) '.summedarea=S.object_feature_size2d(I)/1000/1000*(2*2);'])    %object__area
%         eval(['base_Zooscan(i).d' num2str(fracnb) '.ESD=2*(((S.object_feature_size2d(I)/1000/1000)/pi).^0.5)*(2*2);'])    %object__area
%         
        %%
