function [base_Zooscan,m,zoo_groups,Idlistshort]=process_tsv_H5
...
for i=1:m
    ...
    for fracnb=1:nfrac
        ...
        eval(['base_Zooscan(i).d' num2str(fracnb) '.major=S.object_major_axis(I)/1000;']) %allready in micron => in mm
        eval(['base_Zooscan(i).d' num2str(fracnb) '.minor=S.object_minor_axis(I)/1000;'])
        %eval(['base_Zooscan(i).d' num2str(fracnb) '.area_exc=S.object_area_exc(I)*(pixelsize^2);'])
        eval(['base_Zooscan(i).d' num2str(fracnb) '.summedbiovolume=S.object_size(I)/1000/1000/1000;']) %allready in cubed micron => in mm3
        eval(['base_Zooscan(i).d' num2str(fracnb) '.area=S.object_size2d(I)/1000/1000;'])    %allready in µm2 =>mm2
        eval(['base_Zooscan(i).d' num2str(fracnb) '.summedarea=S.object_size2d(I)/1000/1000;'])    %object__area
        eval(['base_Zooscan(i).d' num2str(fracnb) '.ESD=2*(((S.object_size2d(I)/1000/1000)/pi).^0.5);'])    %object__area
