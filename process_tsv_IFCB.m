...
for i=1:m
    ...
    base_Zooscan(i).Vol=5/1000/1000;%  5ml per sample converted in m3 str2num(cell2mat(unique(S.sample_tot_vol)));
    ...
    pixelsize=(1/str2num((cell2mat(unique(S.process_pixel_to_micron)))))/1000;  % in mm/pixel
    %return
    %%
    ...
    for fracnb=1:nfrac
        ...
        eval(['base_Zooscan(i).d' num2str(fracnb) '.major=S.object_majoraxislength(I)*pixelsize;']) %object_perimmajor
        eval(['base_Zooscan(i).d' num2str(fracnb) '.minor=S.object_minoraxislength(I)*pixelsize;'])
        %eval(['base_Zooscan(i).d' num2str(fracnb) '.area_exc=S.object_area_exc(I)*(pixelsize^2);'])
        eval(['base_Zooscan(i).d' num2str(fracnb) '.summedbiovolume=S.object_summedbiovolume(I)*(pixelsize^3);'])
        eval(['base_Zooscan(i).d' num2str(fracnb) '.area=S.object_area(I)*(pixelsize^2);'])    %object__area
        eval(['base_Zooscan(i).d' num2str(fracnb) '.summedarea=S.object_summedarea(I)*(pixelsize^2);'])    %object__area
